...
(function (w) {
  ...
  // Lấy bản ghi latest tương ứng 1 location_id từ /obs/latest
  ...
  w.fetchAlertSummary = async function (location_id) {
    ...
    try {
      ...
      // Không có summary và cũng không có latest -> trả null
      if (!summary && !latestObs) {
        return null;
      }

      // Nếu không có summary nhưng có latest -> tạo object tối thiểu cho panel
      if (!summary) {
        summary = {
          found: true,
          obs: {},
  ...
  // ================== HELPERS ==================
  // Chuyển đổi độ gió sang hướng chữ
  ...
  w.updateAlertPanel = function (data) {
    ...
    const obs = data.obs || {};
    const todayText =
      data.today && data.today.summary_text
        ? data.today.summary_text
        : "Không có thông tin dự báo.";
    const currentText =
      data.current && data.current.summary_text
        ? data.current.summary_text
        : "";
    ...
    const overallComment =
      ...
      "Thời tiết ổn định, không có nguy cơ đáng kể.";

    // --- RENDER 1: OVERVIEW ---
    let iconOverview = `<i class="fa-solid fa-check-circle"></i>`;
    if (overallLevel === "warning") {
      iconOverview = `<i class="fa-solid fa-triangle-exclamation"></i>`;
    }
    if (overallLevel === "danger") {
      iconOverview = `<i class="fa-solid fa-skull-crossbones"></i>`;
    }
    ...
    // Gió & Hướng gió
    const windStr =
      obs.wind_ms != null ? `${obs.wind_ms.toFixed(1)} m/s` : "--";
    const windDirVal = obs.wind_dir_deg != null ? obs.wind_dir_deg : 0;
    const windDirText =
      obs.wind_dir_deg != null ? getWindDirection(obs.wind_dir_deg) : "--";

    // Mưa & Độ ẩm
    const rainStr =
      obs.precip_mm != null ? `${obs.precip_mm.toFixed(1)} mm` : "0 mm";
    const humidityStr =
      obs.rel_humidity_pct != null
        ? `${obs.rel_humidity_pct.toFixed(0)}%`
        : "--%";
    ...
    const pressureStr =
      obs.surface_pressure_hpa != null
        ? `${obs.surface_pressure_hpa.toFixed(0)} hPa`
        : "--";
    ...
    } else if (obs.cloudcover_pct > 80) {
      weatherIcon = `<i class="fa-solid fa-cloud" style="color:#64748b"></i>`;
    } else if (obs.cloudcover_pct > 30) {
      ...
      <div class="alert-grid">
        ...
        <div class="alert-meta-item">
          ...
          <div class="alert-meta-value-row">
             <div class="wind-arrow-box" style="transform: rotate(${windDirVal}deg);">
    ...
    hazards.forEach((h) => {
      ...
      if (h.type === "heavy_rain") {
        hazIcon = `<i class="fa-solid fa-cloud-showers-water"></i>`;
      }
      if (h.type === "heat") {
        hazIcon = `<i class="fa-solid fa-temperature-arrow-up"></i>`;
      }
      if (h.type === "strong_wind") {
        hazIcon = `<i class="fa-solid fa-wind"></i>`;
      }
      ...
      card.innerHTML = `
        ...
        ${
          advices.length
            ? `<ul class="hazard-advices">${advices
                .map((a) => `<li>${a}</li>`)
                .join("")}</ul>`
            : ""
        }
